...
randomize();
var enemy_x = irandom_range(oVarGame.height_buffer,room_width-oVarGame.height_buffer);
var enemy_y = irandom_range(oVarGame.height_buffer,room_height-oVarGame.height_buffer); 

with(oPlayer)
{
	while(distance_to_point(enemy_x,enemy_y)<50)
	{
		var enemy_x = irandom_range(oVarGame.height_buffer,room_width-oVarGame.height_buffer);
		var enemy_y = irandom_range(oVarGame.height_buffer,room_height-oVarGame.height_buffer); 
	}
...
if(type <50){
	enemy_bar_inst = instance_create_layer(enemy_x,enemy_y,"layer_enemies", oGate_Green); 
	type = 0;
}
else if(type >=50)
{
	enemy_bar_inst = instance_create_layer(enemy_x,enemy_y,"layer_enemies", oGate_Purple); 
	type =1;
}
...
enemy_bar_inst.rotation_speed = rotate_speed/10;
